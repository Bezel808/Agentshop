...
│   ├── orchestration/       # Unified AgentOrchestrator
│   ├── perception/          # Verbal / Visual 感知
│   └── tools/               # Shopping tools + MCP browser tools
│
├── configs/
...
│
├── tests/                   # pytest 测试目录（主入口）
├── scripts/
│   ├── experiments/         # 批量实验/推荐提取脚本
│   └── capture_baseline_snapshot.sh
├── aces_ctl.sh              # 控制脚本
├── start_web_server.py      # Web 服务
├── run_browser_agent.py     # Agent 唯一入口（VLM 理解需求 + verbal/visual + 推荐一商品）
├── manage_datasets.py       # 数据集管理（统一入口）
├── test_*.py                # 兼容入口（转发到 tests/）
│
├── README.md
├── docs/
│   ├── guides/              # QUICKSTART / EXPERIMENT_GUIDE
│   ├── reference/           # DATA_SOURCES / CONDITION_REFERENCE / PROJECT_STRUCTURE
│   ├── architecture/
│   ├── migration/
│   └── baseline/
└── examples/                # 示例脚本
...
| `aces/config/prompt_profiles.py` | Prompt engineering profile 配置 |

...
python -u run_browser_agent.py \
  ...
  --perception visual \
  --query mousepad \
  --prompt-profile robust_compare \
  --env-backend playwright \
  --trace-output logs/run_trace.jsonl
```

- `--perception verbal`：文本模式（不启动浏览器）
- `--env-backend api|playwright|mcp|auto`：切换环境后端（默认 auto）
- `--prompt-profile robust_compare|active_explore|fast_decide`：切换 prompt 工程风格
- `--eval-suite <suite>`：批量跑 query 套件并输出对比报告
- `--legacy-fallback`：启用旧版文本猜工具模式（默认关闭）
- `--once`：分析一次后退出
...
**速查**：[CONDITION_REFERENCE.md](../reference/CONDITION_REFERENCE.md)
